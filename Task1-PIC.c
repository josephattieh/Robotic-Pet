...
{       
    ...
    float distance;
    int digital;
    LCD_String_xy(1,1,"Program start");
    __delay_ms(500);
    LCD_Clear();
    while(1)
    {
	...
	//Convert digital value into analog voltage/
        ...
        distance = distance*2.54*1000/9.8;  
	//It is used to convert integer value to ASCII string/
        sprintf(s,"%.1f",distance);
        LCD_String_xy(1,1,s);//Send string data for printing/
       
        while(distance < 19){
            
            goBackward(1);
            LCD_String_xy(2,1,"Greater");
            digital = ADC_Read(0);
        //distance = digital*1023;
	//Convert digital value into analog voltage/
